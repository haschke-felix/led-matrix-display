...
#define LAST_LETTER '~'
static const byte LETTERS[] PROGMEM = {
...
static const int PTN_LETTERS[] PROGMEM{
...
static const byte col_order[] = {5, 0, 2, 3, 1, 4, 6, 7};
static const byte row_order[] = {7, 6, 5, 4, 3, 2, 1, 0};

MatrixDisplay::MatrixDisplay(const byte height, const byte width) : height_(height), width_(width), pending_(false), brigthness_(9)
{
...
void MatrixDisplay::setRow(const byte row, const int value)
{
	if (row > height_) return;
	getPendingBuffer()[row] = orderCols(value);
}

void MatrixDisplay::show(const bool take_over)
{
	pending_ = !pending_;
	if (take_over)
	{
...
void MatrixDisplay::clearColumns(byte start, byte end, const byte value)
{
...
int MatrixDisplay::setString(const char *s, int column, const char spacing, const byte offset)
{
...
int MatrixDisplay::width(const char ch) { return letterWidth(ch); }
